...
public abstract class TimerProvider : IAssignmentProvider
{
    ...
    /// 任务类型
    /// </summary>
    protected abstract AssignmentType AssignmentType { get; }

    public Task<List<AssignmentDefine>> GetCanReceiveListAsync(AssignmentContext context)
    {
        //先获取到对应任务定义列表
        ...
        var assignmentFilterIds = context.CurrentUserAssignments
            .Where(x => 
                        //正在进行的，已经完成，要去掉
                        x.AssignmentState == AssignmentStatus.InProgress ||
                        x.AssignmentState==AssignmentStatus.Completed||
                        //已结束，但是还没过期，要去掉
                        (x.AssignmentState == AssignmentStatus.Closed && !AssignmentType.IsExpired(x.EndTime!.Value)))
