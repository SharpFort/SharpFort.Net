...
public class AssignmentManager : DomainService
{
    ...
    public readonly ISqlSugarRepository<Assignment> _assignmentRepository;
    public readonly ISqlSugarRepository<AssignmentDefine> _assignmentDefineRepository;
    private readonly ILocalEventBus _localEventBus;

    public AssignmentManager(IEnumerable<IAssignmentProvider> assignmentProviders,
        ISqlSugarRepository<Assignment> assignmentRepository,
        ISqlSugarRepository<AssignmentDefine> assignmentDefineRepository, ILocalEventBus localEventBus)
    {
    ...
    public async Task AcceptAsync(Guid userId, Guid asignmentDefineId)
    {
        ...
        if (canReceiveList.Select(x => x.Id).Contains(asignmentDefineId))
        {
            ...
            entity.AssignmentState = AssignmentStatus.InProgress;
    ...
    /// <returns></returns>
    public async Task<List<AssignmentDefine>> GetCanReceiveListAsync(Guid userId)
    {
        var context = await GetAssignmentContext(userId);
        var output = new List<AssignmentDefine>();
    ...
    public async Task ExpireTimeoutAsync()
    {
        var progressEntities = await _assignmentRepository._DbQueryable
            .Where(x => x.AssignmentState == AssignmentStatus.InProgress)
            .ToListAsync();

        var needUpdateEntities = new List<Assignment>();
