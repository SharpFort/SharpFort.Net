...
namespace Yi.Framework.Bbs.Domain.Managers
{
    ...
    public class BankManager : DomainService
    {
        ...
        private ISqlSugarRepository<BankCard> _repository;
        private ILocalEventBus _localEventBus;
        private ISqlSugarRepository<InterestRecords> _interestRepository;
        private IBankValueProvider _bankValueProvider;
        public BankManager(ISqlSugarRepository<BankCard> repository, ILocalEventBus localEventBus, ISqlSugarRepository<InterestRecords> interestRepository, IBankValueProvider bankValueProvider)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<InterestRecords> CreateInterestRecordsAsync()
        {
            ...
            var entity = new InterestRecords(thirdPartyValue, currentValue, isFluctuate);
        ...
        {
            var entities = Enumerable.Range(1, cardNumber).Select(x => new BankCard(userId)).ToList();
        ...
        public async Task DrawMoneyAsync(Guid cardId)
        {
            ...
            if (entity.BankCardState == BankCardStatus.Idle)
            {
                throw new UserFriendlyException("当前银行卡状态不能提款");
            }

            //这里其实不存在这个状态，只有等待状态，不需要去主动触发，前端判断即可
            if (entity.BankCardState == BankCardStatus.Full)
            {
                throw new UserFriendlyException("当前银行卡状态不能存款");
            }

            //可以提款
            if (entity.BankCardState == BankCardStatus.Waiting)
            {
