...
// 实体扩展类 MenuEntityExtensions 中写的 Vue3RuoYiRouterBuild 逻辑，请按以下方式迁移：
// 新建应用服务方法：
// 在 MenuAppService 中创建一个方法 GetUserRoutersAsync(Guid userId)。
// 数据获取：
// 使用 _menuRepository.GetListAsync(...) 获取该用户的所有菜单实体。
// DTO 转换：
// 将实体的 List<Menu> 转换为前端需要的 List<Vue3RouterDto>。
// 这就是原扩展方法代码应该放置的地方。
// 可以利用 AutoMapper 或手动映射。
