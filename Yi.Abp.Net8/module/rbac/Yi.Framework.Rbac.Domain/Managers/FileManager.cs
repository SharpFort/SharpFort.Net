...
public class FileManager : DomainService, IFileManager
{
    ...
    public FileManager(IGuidGenerator guidGenerator, IRepository<FileStorage> repository,
    ...
    public async Task<List<FileStorage>> CreateAsync(IEnumerable<IFormFile> files)
    {
        if (files.Count() == 0)
        {
        ...
        //批量插入
        List<FileStorage> entities = new();
        foreach (var file in files)
        {
            FileStorage data = new(_guidGenerator.Create(), file.FileName, (decimal)file.Length / 1024);
    ...
    public async Task SaveFileAsync(FileStorage file, Stream fileStream)
    {
