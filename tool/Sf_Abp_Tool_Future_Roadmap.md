# SharpFort.Tool 功能演进分析与路线图

## 1. 现状评估
目前的 `SharpFort.Tool` 包含以下四个核心命令：
*   `new`: 创建项目/模块 (脚手架)。
...
ABP CLI 是一个非常成熟的工具，但对于 Sf Framework (SharpFort) 来说，大部分功能是冗余的。我们需要通过“剔除法”来筛选值得借鉴的功能。

| ABP CLI 命令 | 功能描述 | Sf Framework 借鉴价值 | 理由 |
| :--- | :--- | :--- | :--- |
...
| `login/logout` | 商业版登录 | ❌ 无需借鉴 | Sf 目前是开源项目，不需要鉴权逻辑。 |
...
#### 1. `update` 命令 (强烈推荐)
*   **场景**: 框架发布了新版本 `SharpFort 2.0.0`。
*   **功能**: 扫描解决方案目录下所有的 `.csproj`，找到所有引用 `Sf.*` 或 `SharpFort.*` 的包，将其版本号更新到指定版本或最新版。
*   **命令设计**: 
    ```bash
...
*   **命令设计**:
    ...
    ```
*   **价值**: 这将是 Sf Framework 前后端分离开发的杀手级功能。
...
通过增加这两个命令，`SharpFort.Tool` 将从一个简单的“下载器”转变为一个真正的“生产力工具”。
