# Sf框架模块开发实战总结 - FileManagement模块

## 1. 模块开发流程概览

本次 `SharpFort.FileManagement` 模块的开发遵循 DDD（领域驱动设计）原则，并充分利用了 Sf 框架的基础能力。以下是将此次经验应用到未来模块开发的标准化流程。
...
*   **仓储接口**: 推荐使用 `ISqlSugarRepository<TEntity, TKey>` 而不是标准的 ABP `IRepository`。
    *   **原因**: Sf框架封装的 `ISqlSugarRepository` 提供了直接访问 `_DbQueryable` 的能力，能使用 SqlSugar 强大的查询语法（如 `WhereIF`, `ToPageListAsync`），同时兼容 ABP 的仓储接口。

### 2.2 应用服务 (Application Layer)

Sf 框架提供了强大的基类，极大地减少了 CRUD 代码量。
...
*   **分页查询基类**:
    ...
    *   **PagedAllResultRequestDto**: Sf 框架增强版，增加了 `StartTime`, `EndTime` 时间范围查询，以及更方便的 `OrderByColumn` / `IsAsc` 排序字段。**推荐后台管理列表查询使用此基类。**
*   **实体 DTO**:
...
本次开发中，我们采用了 Casbin 策略鉴权，这是 Sf 框架的核心鉴权机制。
...
1.  **DbContext 命名冲突**:
    *   **问题**: 模块内部定义了 `SfDbContext`，与框架核心 `SharpFort.SqlSugarCore.SfDbContext` 重名，导致 DI 注入时出现歧义。
    *   **解决**: 模块内的 DbContext 应具有唯一名称，如 `FileManagementDbContext`。
    *   **经验**: 永远不要使用通用的 `SfDbContext` 作为模块 DbContext 的名称。
...
3.  **模块依赖关系**:
    *   **Application 层**: 必须依赖 `SharpFortDddApplicationModule` 以使用 `SfCrudAppService`。
    *   **SqlSugarCore 层**: 尽量保持独立，不要依赖 RBAC 等其他业务模块，除非有直接的实体关联。本次开发中我们移除了不需要的 RBAC 依赖，保持了模块的纯净。

4.  **Web 层集成**:
    *   **Controller 注册**: 必须在 `SfAbpWebModule.cs` 中使用 `ConventionalControllers.Create` 注册模块的 API，否则 Swagger 中不会显示接口。
...
- [ ] 应用服务继承 `SfCrudAppService` (普通) 或 `SfCacheCrudAppService` (缓存)
...
此文档旨在帮助您在未来的模块开发中少走弯路，依此流程可快速构建高质量、符合 Sf 框架规范的业务模块。
