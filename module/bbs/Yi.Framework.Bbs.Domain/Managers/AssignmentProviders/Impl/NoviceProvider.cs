...
{
    public async Task<List<AssignmentDefine>> GetCanReceiveListAsync(AssignmentContext context)
    {
        List<AssignmentDefine> output = new List<AssignmentDefine>();
        //新手任务是要有前置依赖关系的，链表类型依赖
        //先获取到对应任务定义列表，新手任务
        var assignmentDefines = context.AllAssignmentDefine.Where(x => x.AssignmentType == AssignmentType.Novice)
            .ToList();
        ...
        if (context.CurrentUserAssignments
            ...
            .Any(x => x.AssignmentState == AssignmentStatus.InProgress))
        {
            return output;
        }
        ...
        var assignmentFilterIds = context.CurrentUserAssignments
            ...
            .Where(x =>
                //已经完成的
                x.AssignmentState == AssignmentStatus.Closed ||
