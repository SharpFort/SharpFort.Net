...
{
    public class ArticleRepository : SqlSugarRepository<Article,Guid>, IArticleRepository,ITransientDependency
    {
        public ArticleRepository(ISugarDbContextProvider<ISqlSugarDbContext> sugarDbContextProvider) : base(sugarDbContextProvider)
        {
        }

        public async Task<List<Article>> GetTreeAsync(Expression<Func<Article, bool>> where)
        {
