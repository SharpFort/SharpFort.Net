...
using Yi.Framework.CasbinRbac.Application.SignalRHubs;
...
namespace Yi.Framework.CasbinRbac.Application.Services
{
    ...
    /// Notice服务实现
    /// </summary>
    public class NoticeService : YiCrudAppService<Notice, NoticeGetOutputDto, NoticeGetListOutputDto, Guid, NoticeGetListInput, NoticeCreateInput, NoticeUpdateInput>,
       INoticeService
    {
        private ISqlSugarRepository<Notice, Guid> _repository;
        private IHubContext<NoticeHub> _hubContext;
        public NoticeService(ISqlSugarRepository<Notice, Guid> repository, IHubContext<NoticeHub> hubContext) : base(repository)
        {
        ...
        public override async Task<PagedResultDto<NoticeGetListOutputDto>> GetListAsync(NoticeGetListInput input)
        {
            ...
            var entities = await _repository._DbQueryable.WhereIF(input.NoticeType is not null, x => x.Type == input.NoticeType)
