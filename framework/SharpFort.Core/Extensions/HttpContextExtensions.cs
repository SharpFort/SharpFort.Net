...
namespace SharpFort.Core.Extensions
{
    /// <summary>
    /// HttpContext扩展方法类
    /// </summary>
    public static class HttpContextExtensions
    {
        ...
        /// <remarks>
        /// 使用场景：
        /// - 图片、PDF 等文件在浏览器中直接预览（inline 方式）
        /// - 文件名会进行 URL 编码以支持中文等特殊字符
        ///
        /// 响应头格式：Content-Disposition: inline;filename={encodedFileName}
        ///
        /// 注意：使用索引器赋值而非 Add()，避免重复键导致 ArgumentException
        /// </remarks>
        ...
        /// <remarks>
        /// 使用场景：
        /// - 文件下载（附件方式，浏览器弹出"另存为"对话框）
        /// - 文件名会进行 URL 编码以支持中文等特殊字符
        ///
        /// 响应头格式：Content-Disposition: attachment;filename={encodedFileName}
        ///
        /// 注意：使用索引器赋值而非 Add()，避免重复键导致 ArgumentException
        /// </remarks>
